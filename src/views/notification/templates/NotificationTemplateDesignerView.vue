...
onMounted(async () => {
  await getTemplate(`/v0/notification/templates/detail/companyId/${auth.user?.company.companyId}/userId/${auth.user?.id}/templateId/${templateId}`);

  if (template.value) {
    editor.value = grapesjs.init({
      container: '#gjs',
      fromElement: false,
      height: '100%', // Changed to 100% so it fills the flex container
      width: 'auto',
      storageManager: false,
      plugins: [grapesjsmjml],
      pluginsOpts: {
        'grapesjs-mjml': {
          resetBlocks: false, // Don't reload blocks every time
        }
      },
      blockManager: {
        appendTo: '#blocks-container', // Matches the ID in <template>
      }
    });
    ...
    if (contentJson && Object.keys(contentJson).length > 0) {
      ...
      // CRITICAL: MJML must have this root to allow dragging into it
    ...
    // 3. FORCE RE-RENDER
    // Sometimes GrapesJS initializes before the DOM is fully ready for the blocks
    nextTick(() => {
      if (editor.value) {
...
const save = async () => {
  ...
  const contentCompiled = editor.value.getHtml(); // or getMjml() if using mjml plugin specifically for raw mjml
  ...
  await updateTemplate(`/v0/notification/templates/update/companyId/${auth.user?.company.companyId}/userId/${auth.user?.id}/templateId/${templateId}`, payload);
};
...
<style scoped>
/* Ensure the full screen is used but nothing overflows */
.designer-container {
  height: 100vh;
  width: 100vw;
  display: flex;
...
.editor-main {
  display: flex;
  flex: 1; /* Takes all remaining height below header */
  overflow: hidden;
  position: relative;
}
...
/* 1. Hide the default GrapesJS top bar so it doesn't block your Vue header */
:deep(.gjs-pn-views),
...
/* 2. FORCE the toolbar to be visible and on top */
:deep(.gjs-toolbar) {
...
/* 3. Ensure the high-light box appears when selecting MJML components */
:deep(.gjs-highlighter) {
  pointer-events: none;
}

/* 4. Position the editor canvas so it doesn't start 'under' the header */
:deep(.gjs-cv-canvas) {
  width: 100% !important;
