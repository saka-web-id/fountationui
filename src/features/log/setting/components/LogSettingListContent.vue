...
onMounted(async () => {
  ...
  console.log("URL :", '/v0/logs/setting/list/companyId/'+auth.user?.company.companyId + "/userId/" + auth.user?.id);
...
const goToEdit = (companyIdParam: number) => {
  router.push({ name: 'logsettingedit', params: { companyIdParam } });
};
            ...
            <div class="ms-2 me-2 mt-2 mb-2">
              <table class="table">
                ...
                <tr>
                  ...
                  <th>{{ t('textField.format') }}</th>
                  <th>{{ t('textField.enabled') }}</th>
                  <th class="text-center">{{ t('textLabel.action') }}</th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="d in data" :key="d.logSettingId">
                  <td>{{ d.logSettingEndpoint  }}</td>
                  <td>{{ d.logSettingMethod }}</td>
                  <td>{{ d.logSettingLogFormat }}</td>
                  <td>{{ d.logSettingEnabled }}</td>
