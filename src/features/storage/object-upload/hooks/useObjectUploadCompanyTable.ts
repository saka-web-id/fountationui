...
export function useObjectUploadCompanyTable() {
    const { t } = useI18n();
    ...
    const goToUploads = (companyId: number) => {
        router.push({ name: 'objectstorageuploadlist', params: { valueCompanyId: companyId } });
    };
        ...
        columnHelper.display({
            ...
            cell: info => h('div', { class: 'btn-group' }, [
                h('button', {
                    ...
                    onClick: () => goToUploads(info.row.original.companyId)
                }, t('button.view'))
            ]),
        }),
    ];
