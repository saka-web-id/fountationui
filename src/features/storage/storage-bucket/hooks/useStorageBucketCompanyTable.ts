...
export function useStorageBucketCompanyTable() {
    const { t } = useI18n();
    ...
    const goToBuckets = (companyId: number) => {
        router.push({ name: 'objectstoragebucketlist', params: { valueCompanyId: companyId } });
    };
        ...
        columnHelper.display({
            ...
            cell: info => h('div', { class: 'btn-group' }, [
                h('button', {
                    ...
                    onClick: () => goToBuckets(info.row.original.companyId)
                }, t('button.view'))
            ]),
        }),
    ];
