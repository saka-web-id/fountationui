...
import { useTemplateCompanyTable } from '../hooks/tables/useTemplateCompanyTable';

const router = useRouter();
const viewMode = ref<'list' | 'grid'>('list');
const { table, globalFilter, fetchData, t } = useTemplateCompanyTable();
  ...
  <section class="pt-2 pb-2">
    <div class="container">
      <ol class="breadcrumb ms-2 me-2">
        ...
        <li class="breadcrumb-item active"><span class="active">{{ t('textLabel.template') }}</span></li>
      </ol>
      <div class="card mb-3 border-0 shadow-sm">
        <div class="card-body">
          <div class="row d-flex justify-content-between align-items-center mb-4">
            <div class="col-auto">
              <h4 class="mb-0">{{ t('textLabel.company', 2) }}</h4>
            </div>
          ...
          <div v-else>
            <BaseGridView :table="table">
              <template #item="{ row }">
                <div class="card h-100 border-0 shadow-sm hover-shadow transition-all" @click="goToTemplateList(row.original.companyId)" style="cursor: pointer;">
                  <div class="card-body text-center py-4">
                    ...
                    <p class="card-text text-muted small">ID: {{ row.original.companyId }}</p>
