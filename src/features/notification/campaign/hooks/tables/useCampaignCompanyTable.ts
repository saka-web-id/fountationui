...
export function useCampaignCompanyTable() {
    const { t } = useI18n()
    ...
    const goToCampaigns = (companyId: number) => {
        router.push({ name: 'notificationCampaignList', params: { companyIdParam: companyId } });
    };
        ...
        columnHelper.display({
            ...
            cell: info => h('div', { class: 'btn-group' }, [
                h('button', {
                    ...
                    onClick: () => goToCampaigns(info.row.original.companyId)
                }, t('button.view'))
            ]),
        }),
    ];
