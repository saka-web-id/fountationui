...
import { useEmailCompanyTable } from '~/features/notification/settings/email/hooks/tables/useEmailCompanyTable.ts';
...
const goToEdit = (companyIdParam: number) => {
    router.push({ name: 'notificationlistemail', params: { companyIdParam } });
};
  ...
  <section class="pt-2 pb-2">
    <div class="container">
      <ol class="breadcrumb ms-2 me-2">
        ...
        <li class="breadcrumb-item active"><span class="active">{{ t('textLabel.emailSetting') }}</span></li>
      </ol>
      <div class="card mb-3 border-0 shadow-sm">
        <div class="card-body">
          <div class="row d-flex justify-content-between align-items-center mb-4">
            <div class="col-auto">
              <h4 class="mb-0">{{ t('textLabel.emailSetting') }}</h4>
            </div>
          ...
          <div v-else>
            <BaseGridView :table="table">
              <template #item="{ row }">
                <div class="card h-100 border-0 shadow-sm hover-shadow transition-all" @click="goToEdit(row.original.companyId)" style="cursor: pointer;">
