...
export function useSmsSettingTable(companyIdParam: string | string[]) {
    const { t } = useI18n()
    ...
    const goToEdit = (providerIdParam: number, companyIdParam: string | string[]) => {
        router.push({ name: 'notificationdetaileditsms', params: { companyIdParam, providerIdParam } });
    };

    const onTestNotification = (providerIdParam: number, companyIdParam: string | string[]) => {
        router.push({ name: 'notificationtestsms', params: { companyIdParam, providerIdParam } });
    };
    ...
    const fetchData = async () => {
        await get(`/v0/notification/provider/list/companyId/${auth.user?.company.companyId}/userId/${auth.user?.id}/valueCompanyId/${companyIdParam}/notificationType/SMS`)
    }
